        ...
        for epoch in range(nepochs):
            ...
            for batch in dataset:
                ...
                if self.batch_count % batch_loss_every  == 0:
                    self.metrics_history.update('train/batch_loss', self.epoch, loss.item(), x=self.batch_count)
                    if(self.writer):
            ...
            if (epoch % eval_every == 0) and (self.evaluator is not None):
                metrics = {}
                metrics['train'] = self.evaluator.getMetrics(dataset, mask=True)
                ...
                if(self.writer):
                    for tag in metrics:
                        for metric in metrics[tag]:
                            self.writer.add_scalar("{}/{}".format(tag, metric), metrics[tag][metric], self.epoch)
            
            # checkpoint
            if checkpoint_every and ((self.epoch+1) % checkpoint_every == 0):
                fname = ospj(self.checkpoint_path, "{}.{}.tar".format(self.model_name, epoch + 1))
